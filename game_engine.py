...
def paint_game(juego):
    ...
    for k in range(0, 20):
        for j in range(0,2):
            ...
            tmpx.rect.left = 32 * k
            obj.append(tmpx)
    farolap = Pintable(juego.farola, juego.farola.get_rect())
    farolap.rect.centerx = juego.config.getWindowANCHO() / 2
    farolap.rect.bottom = juego.config.getWindowALTO() - 55
    obj.append(farolap)
...
def game_loop(juego):
    while True:
        ...
        if controles[K_RIGHT]:
            juego.player.update_velocity([2.0, juego.player.velocity[1]])
